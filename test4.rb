...
random_umbers.each do |num|
  all_grid_array.each do |grid|
    grid.each do|line|
      ...
      if verification_line(grid) || verification_collumn(grid,0) || verification_collumn(grid,1) || verification_collumn(grid,2) || verification_collumn(grid,3) || verification_collumn(grid,4)
        left_numbers = 0
